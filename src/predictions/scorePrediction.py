...
def app():
    ...
    try:
        model = joblib.load(open('Model/score_prediction_model.pkl', 'rb'))
    except (FileNotFoundError, pickle.UnpicklingError) as e:
        st.error(f"Error loading score prediction model: {str(e)}")
    ...
    with col1:
        batting_team = st.radio('Batting Team At The Moment', TEAMS)
    with col2:
        bowling_team = st.radio('Bowling Team At The Moment', TEAMS)
